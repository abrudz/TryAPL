...
(function (root) {
  ...
  // --- Configurable prefix keys -----------------------------------------------
  // The keys that start "prefix" glyph entry (backtick + legacy alternates).
  // User-overridable and persisted; consumed by lib/tiolb.js.
  var PREFIX_DEFAULT = "`½²^º§ùµ°";
  function getPrefixKeys() {
    try { return localStorage.getItem("prefixKeys") || PREFIX_DEFAULT; } catch (e) { return PREFIX_DEFAULT; }
  }
  function setPrefixKeys(s) {
    try {
      if (s) localStorage.setItem("prefixKeys", s);
      else localStorage.removeItem("prefixKeys");
    } catch (e) {}
  }
  ...
  root.Glyphs = {
    ...
    PREFIX_DEFAULT: PREFIX_DEFAULT, getPrefixKeys: getPrefixKeys, setPrefixKeys: setPrefixKeys
  };
