...
function loadTryAPL () {
	...
	// Get symbols / glyphs for Primer help (parser lives in lib/glyphs.js)
	Glyphs.loadElements().then(r=>{symbols=r.symbols;elements=r.elements})
	ngn_lb_bs=$$(".ngn_lb b")
...
// Where to send /Exec. Same-origin on the canonical deployments and the local
// dev server (which proxies it); cross-origin to the public backend elsewhere
// (e.g. a GitHub Pages mirror at abrudz.github.io/TryAPL) — the backend sends
// permissive CORS headers, so a static front end can reach it directly.
var EXEC_URL = (function () {
	var h = location.hostname;
	if (/(^|\.)tryapl\.(org|com)$/.test(h) || /\.dyalog\.com$/.test(h) || h === "localhost" || h === "127.0.0.1" || h === "") return "Exec";
	return "https://tryapl.org/Exec";
})();

// Low-level backend call: POST a state array to the Exec endpoint and resolve
// with the parsed response state array (or reject with {status, statusText}).
// Shared by jarvisProcess and, later, the workspace explorer / embed widget.
function postExec(st) {
	return new Promise(function(resolve, reject){
		...
		log("state out: " + JSON.stringify(st))
		xhttp.send(JSON.stringify(st))
	})
}
...
function jarvisProcess(currentLine) {
	log("jarvisProcess")
	state[3]=currentLine
	return postExec(state).then(function(newState){
		state = newState
		result = state[3].join("\n")
		// Rich output: if the result is a ∆RICH∆ sentinel, render it in the dock
		// and replace the printed text with a short placeholder. No sentinel ->
		// state[3]/result are untouched and the session behaves exactly as before.
		var rich = window.RichOutput ? RichOutput.intercept(state[3]) : null
		if (rich) {
			RichOutput.render(rich)
			state[3] = ["[rich: " + rich.mediatype + " ↓]"]
			result = state[3].join("\n")
		}
		...
		// Notify optional features (workspace explorer, embed widget) that the
		// workspace may have changed, carrying the result text. Harmless if
		// nothing is listening.
		try { document.dispatchEvent(new CustomEvent("tryapl:executed", {detail: {result: response}})) } catch (e) {}
		return response
	}).catch(e => {
			log(e)
			if (0 == e.status) {
...
// The tab icon/text switch is now handled in CSS via a container query on
// #leftPane (see style.css). Kept as a no-op so existing callers are harmless.
