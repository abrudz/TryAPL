...
function jarvisProcess(currentLine) {
	...
	return postExec(state).then(function(newState){
		...
		// Notify optional features (e.g. the workspace explorer) that the
		// workspace may have changed. Harmless if nothing is listening.
		try { document.dispatchEvent(new CustomEvent("tryapl:executed")) } catch (e) {}
		return response
	}).catch(e => {
			log(e)
			if (0 == e.status) {
